...
if ($Dataset -notmatch "^(blender_synthetic|d_nerf)$") {
    Write-Host "WARN: Invalid dataset: $Dataset" -ForegroundColor Yellow
    exit 1
}
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "====================================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "OK: Dataset location: ./data/$Dataset" -ForegroundColor Cyan
    Write-Host "OK: Ready for benchmarking!" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "ERROR: Dataset download failed" -ForegroundColor Red
    exit 1
}
